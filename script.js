(function () {
  ...
  // Dedicated pixel ID for dual-fire
  ...
  function track(event, params) {
    if (typeof window.fbq === "function") {
      try {
        // Shared pixel: trackSingle keeps it isolated
        window.fbq("trackSingle", "1178133073434960", event, params || {});
      } catch (_) {}
    }
  }

  // Dedicated pixel events — fires only to the dedicated pixel
  function trackDedicated(event, params, eventId) {
    if (!DEDICATED_PIXEL_ID || DEDICATED_PIXEL_ID === 'YOUR_DEDICATED_PIXEL_ID') return;
    if (typeof window.fbq === "function") {
      try {
  ...
  // ViewContent to dedicated pixel on load
  var vcFired = false;
  ...
  detailsForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // Dedicated pixel Schedule with eventId for dedup
      trackDedicated("Schedule", { content_name: SERVICE_NAME }, eventId);

      // Server-side CAPI Schedule (same eventId => deduplicated with browser event)
      try {
        const getCookie = (n) =>
